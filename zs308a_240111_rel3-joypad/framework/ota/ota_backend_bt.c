...
int ota_cmd_h2d_send_image_data(struct svc_prot_context *ctx, uint16_t param_len, int with_crc)
{
	...
	SYS_LOG_INF("%s, buf %p, len %d, with_crc=%d", __func__, ctx->read_buf, ctx->read_len, with_crc);
	...
	/* read data */
	SYS_LOG_INF("%s, svc_prot_get_rx_data buf=%p, len=%d", __func__, (ctx->read_buf + ctx->read_buf_len), seg_len);
...
{
	SYS_LOG_INF("%s, start", __func__);
	int ret = ota_cmd_h2d_send_image_data(ctx, param_len, 1);
	SYS_LOG_INF("%s, ota_cmd_h2d_send_image_data ret = %d", __func__, ret);
	return ret;
}
...
{
	SYS_LOG_INF("%s, start", __func__);
	int ret = ota_cmd_h2d_send_image_data(ctx, param_len, 0);
	SYS_LOG_INF("%s, ota_cmd_h2d_send_image_data ret = %d", __func__, ret);
	return ret;
}
...
int ota_backend_bt_read(struct ota_backend *backend, int offset, uint8_t *buf, int size)
{
	...
	SYS_LOG_INF("%s, offset 0x%x, size %d, buf %p", __func__, offset, size, buf);
...
try_again:
	...
	while (svc_ctx->read_done_len != size) {
		SYS_LOG_INF("%s, cmd=[%02x], svc_ctx->read_done_len=%d, size=%d",
					__func__, processed_cmd, svc_ctx->read_done_len, size);
