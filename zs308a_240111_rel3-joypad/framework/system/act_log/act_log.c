...
int act_log_init(void)
{
    ...
    ctrl->level = CONFIG_ACTLOG_COMPILE_LEVEL;
...
#endif
{
	...
	if(!actlog_ctrl.task_ready){
		printk("[%03d][%c][%s][%d*]:", log_idx, act_loglevel_info[log_data.bit_data.level], actlog_source_name_get(log_data.bit_data.id), log_data.bit_data.line);
		vprintk(fmt, args);
		printk("\r\n");
		log_idx++;
        return;
