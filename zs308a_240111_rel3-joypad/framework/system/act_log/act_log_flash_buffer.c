...
static struct device *flash_buffer_storage_init(int partition_id)
{
    ...
    if (partition_id) {
		...
		}else if(part->storage_id == STORAGE_ID_NAND || part->storage_id == STORAGE_ID_BOOTNAND){
...
void check_save_log_to_flash(void)
{
    ...
    if(val) {
        ...
        if (part->storage_id == STORAGE_ID_NOR || part->storage_id == STORAGE_ID_DATA_NOR) {
            flash_erase(f_dev, part->offset, part->size);
            flash_write(f_dev, part->offset, sram_buf, CONFIG_SIM_FLASH_SIZE);
        } else {
            flash_erase(f_dev, part->offset>>9, part->size>>9);
            flash_write(f_dev, part->offset>>9, sram_buf, CONFIG_SIM_FLASH_SIZE>>9);
        }
