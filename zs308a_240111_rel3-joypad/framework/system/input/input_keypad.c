...
static inline void report_work(int need_report)
{
	if (!need_report)
		return;
...
static void keypad_scan_callback(struct device *dev, struct input_value *val)
{
	...
	SYS_LOG_DBG("%s, type: %d, code:%d, value:%d\n", __FUNCTION__, val->keypad.type, val->keypad.code, val->keypad.value);
	...
	report_work(need_report);

}
static void onoff_scan_callback(struct device *dev, struct input_value *val)
{
	static input_dev_state_t last_value = KEY_VALUE_UP;
	bool need_report = false;
    // int32_t time_ms;		// disable: totally rely on pmu's notification
	...
	case KEY_ONOFF_PMU_LONG_PRESSED:
		keypad->report_press_code = keypad->press_code;
		keypad->press_type = KEY_TYPE_LONG_DOWN;
		if (is_support_hold(keypad->press_code)) {
			keypad->key_hold = true;
...
//	}
	...
	keypad->onoff_handle = key_device_open(&onoff_scan_callback, ONOFF_KEY_DEVICE_NAME);
