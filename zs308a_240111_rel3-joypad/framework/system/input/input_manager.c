...
#ifdef CONFIG_ACTS_DVFS_DYNAMIC_LEVEL
	k_delayed_work_init(&input_manager->boost_work, input_boost_work);
#endif
...
void input_manager_boost(bool boost)
{
#ifdef CONFIG_ACTS_DVFS_DYNAMIC_LEVEL
	unsigned int key;
	bool last_boosted = false;

	key = os_irq_lock();
	last_boosted = input_manager->is_boosted;
	if (boost) {
		input_manager->is_boosted = 1;
	}
	os_irq_unlock(key);
	...
	if (last_boosted != boost) {
		if (boost) {
			...
			k_delayed_work_cancel(&input_manager->boost_work);
			dvfs_set_level(DVFS_LEVEL_HIGH_PERFORMANCE, "input");
		} else {
			//SYS_LOG_INF("input boost delayed stop");
			k_delayed_work_submit(&input_manager->boost_work, K_MSEC(2000));
		}
	}
#endif
}
