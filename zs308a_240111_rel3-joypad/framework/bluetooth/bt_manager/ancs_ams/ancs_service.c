...
struct ancs_app_attr_cmd_t {
	 ...
	 * request
	 */
	uint8_t data[(ANCS_MAX_NOTIF_ATT_LENGTH * 2) + 1];
...
extern void ancs_get_application_attribute_cmd(struct bt_conn *conn)
{
	...
	memset(app_attr_req.data, 0, (ANCS_MAX_NOTIF_ATT_LENGTH * 2) + 1);
...
#ifndef ANCS_USED_CACHE_MSG
	/* Copy the APP ID */
	for (uint16_t loop_count = 0; loop_count < strlen(g_ancs_appId_data); loop_count++){
