...
static void btmgr_hid_send_hid_key(uint16_t hdl, uint8_t report_id, uint8_t func_id)
{
	uint8_t tmp_data[2];

	SYS_LOG_INF("Hid report id 0x%x, func_id 0x%02x", report_id, func_id);

	memset(tmp_data, 0, sizeof(tmp_data));
	/* report id */
	tmp_data[0] = report_id;

	uint8_t shift_left_bits = func_id & 0x07;		// shift_left_bits will only in range[0:7]
	/* push */
	tmp_data[1] = 0x01 << shift_left_bits;
...
void bt_manager_hid_delay_work(os_work *work)
{
	...
	case HID_STATE_WAIT_DELAY_OPT:
		btmgr_hid_send_hid_key(info->hdl, info->hid_report_id, 0x01);
...
static int bt_manager_hid_send_report_id(uint8_t report_id, uint8_t func_id)
{
	struct bt_mgr_dev_info *info;
	uint16_t hdl;

	SYS_LOG_INF("%s, report_id=0x%02x", __func__, report_id);
	...
	if (info->hid_connected) {
		btmgr_hid_send_hid_key(hdl, report_id, func_id);
		info->hid_state = HID_STATE_WAIT_DISCONNECT;
		os_delayed_work_submit(&info->hid_delay_work, mgr_hid_info.disconnect_delay);
...
{
	return bt_manager_hid_send_report_id(0x02, 0x02);		/* 0x09, 0xe9, //USAGE (volume up) */
}

int bt_manager_hid_send_key(uint8_t report_id, uint8_t func_id)
{
	return bt_manager_hid_send_report_id(report_id, func_id);
}
