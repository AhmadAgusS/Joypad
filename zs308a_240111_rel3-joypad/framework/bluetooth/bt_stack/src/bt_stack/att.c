...
extern void ATTDbgLog(u8_t type, struct bt_conn *conn, struct net_buf *buf);
...
static int bt_att_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	if (buf->len < sizeof(*hdr)) {
		BT_ERR("Too small ATT PDU received:%d,%d", buf->len, sizeof(*hdr));
		return 0;
	}

	ATTDbgLog(0, chan->conn, buf);
