...
static void clcc_cb(struct bt_conn *conn, uint32_t idx, uint32_t dir, uint32_t status,
                                        uint32_t mode, uint32_t mpty, uint8_t *number, uint32_t type, uint8_t *name)
{
...
static void hid_event_handler_cb(struct bt_conn *conn, uint8_t event,uint8_t *data, uint16_t len)
{
	...
	if(event == BT_HID_EVENT_GET_REPORT){
		struct bt_hid_report* report = (struct bt_hid_report*)data;
