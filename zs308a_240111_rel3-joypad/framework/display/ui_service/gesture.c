...
static void gesture_fixed(input_dev_runtime_t *runtime)
{
	...
	if (runtime->last_gesture == 0 || runtime->last_gesture == runtime->gesture ||
		(runtime->last_gesture < GESTURE_DROP_LEFT) != (runtime->gesture < GESTURE_DROP_LEFT)) {
		return;
	}
...
static void gesture_scroll_end(input_dev_t * pointer_dev)
{
	...
	if (runtime->view_id != VIEW_INVALID_ID) {
		...
		if (cfg.duration == 0) {
			...
			cfg.duration = (UI_MAX(step_x, step_y) * 16 + 31) / 32;
		}
