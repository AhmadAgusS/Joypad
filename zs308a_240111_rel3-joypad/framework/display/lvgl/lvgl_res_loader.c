...
static void _bitmap_mem_debug_set_default_scene_id(uint32_t scene_id)
{
	...
	//os_printk("RES LEAK INF: load scene 0x%x\n", scene_id);
	...
	while(item)
	{
		...
		{
			os_printk("RES LEAK WARNING: scene 0x%x maybe not unloaded\n", item->scene_id);
			os_printk("unloaded bitmap ids: \n");
...
static void _bitmap_mem_debug_add_bitmap_to_scene(uint32_t scene_id, uint32_t id, uint32_t addr)
{
	...
	for(i=0;i<MAX_BITMAP_CHECKABLE;i++)
	{
		if(item->res_id[i] == id)
		{
			...
			{
				os_printk("RES LEAK WARNING: unmatched addr for same res id %d, previous 0x%x, current 0x%x\n", 
						id, item->addr[i], addr);
			}
...
static void _bitmap_mem_debug_add_string_to_scene(uint32_t scene_id, uint32_t id, uint32_t addr)
{
	...
	for(i=0;i<MAX_STRING_CHECKABLE;i++)
	{
		if(item->res_id[i] == id)
		{
			...
			if(item->addr[i] != addr && item->ref[i] > 1)
			{
				os_printk("RES LEAK WARNING: unmatched addr for same string id %d, previous 0x%x, current 0x%x\n", 
						id, item->addr[i], addr);
			}
...
static void _bitmap_mem_debug_remove_bitmap_from_scene(uint32_t addr)
{
	...
	if(not_found == 1)
	{
		os_printk("RES LEAK WARNING: pic addr 0x%x not found when unload\n", addr);
	}
}
...
static void _bitmap_mem_debug_remove_string_from_scene(uint32_t addr)
{
	...
	if(not_found == 1)
	{
		os_printk("RES LEAK WARNING: string addr 0x%x not found when unload\n", addr);
	}
}
...
static void _bitmap_mem_debug_bitmap_id_check(uint32_t scene_id, bool unload_compact_buffer)
{
	...
	//os_printk("RES LEAK INF: unload scene 0x%x, unload compact %d\n", scene_id, unload_compact_buffer);
	...
	if(!item)
	{
		os_printk("RES LEAK WARNING: scene to be unload not found 0x%x\n", scene_id);
		return;
	}
	...
	for(i=0;i<MAX_BITMAP_CHECKABLE;i++)
	{
		...
		{
			os_printk("RES LEAK WARNING: bitmap maybe leaked, scene 0x%x, res id %d, ref %d\n", item->scene_id, item->res_id[i], item->ref[i]);
		}
	}
	...
	for(i=0;i<MAX_STRING_CHECKABLE;i++)
	{
		...
		{
			os_printk("RES LEAK WARNING: string maybe leaked, res id %d, ref %d\n", sitem->res_id[i], sitem->ref[i]);
		}
	}
