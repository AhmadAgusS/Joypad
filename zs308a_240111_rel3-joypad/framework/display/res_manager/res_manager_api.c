...
compact_buffer_t* _init_compact_buffer(uint32_t scene_id, uint32_t request_size)
{
	...
	if(buffer == NULL)
	{
		os_printk("malloc compact buffer struct failed\n");
		return NULL;
	}
	...
	if(buffer->addr == NULL)
	{
		os_printk("no enough sapce for compact buffer, size %d\n", request_size);
		buffer->addr = ui_mem_alloc(MEM_FB, screen_bitmap_size, __func__);
		if(buffer->addr == NULL)
		{
			os_printk("no extra sapce for scene 0x%x\n", scene_id);
...
#if RES_MEM_DEBUG
	{
		...
		while(check != NULL)
		{
			if(check == buffer)
			{
				os_printk("invalid compact buffer list for buffer 0x%x:\n", buffer);
				break;
			}
			check = check->next;
			check_count++;
			if(check_count > 500)
			{
				os_printk("loophole compact buffer list for buffer 0x%x:\n", buffer);
				break;
			}
		}
		...
		if(check != NULL)
		{
			...
			while(check != NULL)
			{
				os_printk("compact_buffer: 0x%x, addr 0x%x\n", check, check->addr);
				check = check->next;
				check_count++;
				if(check_count > 500)
				{
...
uint8_t* _get_resource_scene_buffer(uint32_t source, uint32_t id, uint32_t size)
{
	...
	else
	{
		...
		if(item == NULL)
		{
			os_printk("error: scene buffer not enough for size %d \n", size);
			res_manager_dump_info();
			return NULL;
		}
...
uint8_t* _get_resource_text_buffer(uint32_t source, uint32_t id, uint32_t size)
{
	...
	else
	{
		...
		if(item == NULL)
		{
			os_printk("error: resource buffer not enough for size 0x%x \n", size);
			res_manager_dump_info();
			return NULL;
		}
		item->source = source;
...
void _free_resource_text_buffer(void* p)
{
	...
	if(item == NULL)
	{
		os_printk("resource text %p not found \n", p);
		return;
	}
	else if((item != NULL)&&(item->ref > 1))
	{
...
uint8_t* _get_resource_bitmap_buffer_compact(uint32_t source, resource_bitmap_t* bitmap, uint32_t size, uint32_t scene_id)
{
	...
	{
		ui_mem_buf = ui_mem_alloc(MEM_FB, size, __func__);
		if(ui_mem_buf == NULL)
		{
			os_printk("cant malloc screen size bitmap buffer from ui mem \n");
			res_manager_dump_info();
			return NULL;
		}
...
static buf_block_t* _malloc_buf_block(uint32_t size)
{
	...
	if(size == screen_bitmap_size)
	{
		...
		if(item == NULL)
		{
			...
			ui_mem_free(MEM_FB, ui_mem_buf);
			ui_mem_total--;
			os_printk("%s %d: error: resource buffer not enough for size %d\n", __func__, __LINE__, size);
			return NULL;
		}
		item->size = sizeof(buf_block_t);
		item->addr = ui_mem_buf;
	...
	else
	{
		...
		if(item == NULL)
		{
			os_printk("error: resource buffer not enough for size %d\n", size);
			return NULL;
		}
...
void _free_resource_bitmap_buffer(void *p)
{
	...
	if(item == NULL)
	{
//		os_printk("resource bitmap not found %p", p);
/*		
		item = bitmap_buffer.head;
		while(item!=NULL)
		{
	...
	else
	{
		...
		else
		{
			...
			else if(is_compact > 1)
			{
				...
				{
					ui_mem_free(MEM_FB, buffer->addr);
					ui_mem_total--;
				}
				else
				{
...
static int _init_pic_search_param(resource_info_t* info, const char* picres_path)
{
	...
	{
		os_printk("no enough memory to store pic res param\n");
		res_manager_dump_info();
		return -1;
	}
	...
	if(res_path == NULL)
	{
		os_printk("no enough memory to store pic res path\n");
		return -1;
	}
	...
	for(i=0;i<=part_count;i++)
	{
		...
		if(i == 0)
		{
			...
			if ( ret < 0 )
			{
				os_printk(" open picture resource file %s failed!\n", res_path);
				mem_free(res_path);
				return -1;
			}
			...
			{
				os_printk("read picture resource header failed\n");
			...
			{
				os_printk("read picture resource header invalid\n");
				fs_close(&info->pic_fp);
				mem_free(res_path);
				return -1;
			}
			...
			if(entry == NULL)
			{
				os_printk("malloc res entries failed\n");
				fs_close(&info->pic_fp);
				mem_free(res_path);
				res_manager_dump_info();
				return -1;
			...
			{
				os_printk("read picture resource entries failed\n");
			...
			{
				os_printk("malloc pic search offsets failed");
				res_mem_free(RES_MEM_POOL_BMP, entry);
				fs_close(&info->pic_fp);
				mem_free(res_path);
				res_manager_dump_info();
		...
		else
		{
			...
			if(info->pic_search_param[i].compress_size == NULL)
			{
				os_printk("malloc pic search compress size failed");
				res_mem_free(RES_MEM_POOL_BMP, entry);
				res_mem_free(RES_MEM_POOL_BMP, info->pic_search_param[i].pic_offsets);
				info->pic_search_param[i].pic_offsets = NULL;
				fs_close(&info->pic_search_param[i].res_fp);
...
static int _read_head( resource_info_t* info )
{
	...
	{
		os_printk("no enough mem for style data\n");
		res_manager_dump_info();
		return -1;
	}
	...
	if(ret < sty_size)
	{
		os_printk("read style file error:%d\n", ret);
	...
	{
		os_printk("read style file invalid 0x%x\n", head->scene_sum);
		free(info->sty_mem);
		info->sty_mem = NULL;
		return -1;
	}
...
int32_t res_manager_set_str_file(resource_info_t* info, const char* text_path)
{
	...
	if( text_path != NULL)
	{
		...
		if ( ret < 0 )
		{
			os_printk(" open text resource file %s failed! \n", text_path);
			goto ERR_EXIT;
		}
		...
		if(res_is_auto_search_files())
		{
			...
			if(file_path == NULL)
			{
				os_printk("malloc file path failed\n");
				goto ERR_EXIT;
			}
			...
			if(partition == NULL)
			{
				os_printk("invalid str file path\n");
				goto ERR_EXIT;
			}
			...
			if(i<0)
			{
				os_printk(" open str file %s failed! \n", text_path);
				goto ERR_EXIT;
			}
		}
		else
		...
		{
			os_printk("malloc info str path failed\n");
			goto ERR_EXIT;
		}
...
resource_info_t* res_manager_open_resources( const char* style_path, const char* picture_path, const char* text_path )
{
    ...
    if ( info == NULL )
    {
        os_printk( "error: malloc resource info failed ! \n" );
        return NULL;
    }
    ...
    if ( style_path != NULL )
    {
		...
		if ( ret < 0 )
	    {
	        os_printk(" open style file %s failed! \n", style_path);
	        goto ERR_EXIT;
	    }
#else
		if(res_is_auto_search_files())
		{
			...
			if(file_path == NULL)
			{
		        os_printk("malloc style file path failed! \n");
		        goto ERR_EXIT;
			}
...
void res_manager_close_resources( resource_info_t* info )
{
	if(info == NULL)
	{
		os_printk("null resource info pointer \n");
		return;
	}
...
//		SYS_LOG_INF("scene %d, %d, %d %d", scene->x, scene->y, scene->width, scene->height);
	...
	else
	{
		...
		if(scene == NULL)
		{
			os_printk("error: cant get scene buffer \n");
			return NULL;
		}
	}	
	os_strace_end_call_u32(SYS_TRACE_ID_RES_SCENE_PRELOAD_1, (uint32_t)id);
...
//    	SYS_LOG_INF("scene_item 0x%x", scenes[i].scene_id);
    ...
    os_printk("cannot find scene id: 0x%x \n", scene_id);
    return NULL;
}
...
int32_t res_manager_set_pictures_regular(resource_info_t* info, uint32_t scene_id, uint32_t group_id, uint32_t subgroup_id, uint32_t* id, uint32_t num)
{
	...
	if(item == NULL)
	{
		os_printk("error: resource buffer not enough for regular info\n");
		res_manager_dump_info();
		return -1;
	}
...
void _unload_bitmap(resource_bitmap_t* bitmap)
{
	if(bitmap == NULL)
	{
		os_printk("%s %d: error: unload null resource \n", __func__, __LINE__);
		return;
	}
	os_mutex_lock(&bitmap_cache_mutex, OS_FOREVER);
...
int _search_res_id_in_files(resource_info_t* info, resource_bitmap_t* bitmap, uint32_t* bmp_pos, uint32_t* compress_size)
{
...
int32_t _load_bitmap(resource_info_t* info, resource_bitmap_t* bitmap, uint32_t force_ref)
{
	...
	{
		os_printk("error: invalid param to load bitmap \n");
		return -1;
	}
	...
	if(bitmap->buffer == NULL)
	{
		os_printk("error: no buffer to load bitmap \n");
		os_mutex_unlock(&bitmap_cache_mutex);
		os_strace_end_call_u32(SYS_TRACE_ID_RES_BMP_LOAD_0, (uint32_t)bitmap->sty_data->id);
		return -1;
	}
...
//	SYS_LOG_INF("\n pres_entry length %d, type %d, offset 0x%x \n", pres_entry->length, pres_entry->type, pres_entry->offset);
	...
	if (compress_size > 0)
	{
		...
		{
			os_printk("error: no buffer to load compressed bitmap \n");
			os_mutex_unlock(&bitmap_cache_mutex);
		...
		{
			int i;
			os_printk("bmp dump pos for %s: 0x%x\n", info->sty_path, bmp_pos);
			for(i=0;i<4;i++)
			{
				os_printk("%d: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", i, compress_buf[8*i], compress_buf[8*i+1], compress_buf[8*i+2],
	...
	if(ret < bmp_size)
	{
		os_printk("bitmap load error %d, compress_buf %p, compress_size %d, styid 0x%x, id %d, w %d, h %d, bmp_size %d, buffer %p, bmp_pos 0x%x\n", 
...
int _load_string(resource_info_t* info, resource_text_t* text)
{
	...
	{
		os_printk("error: invalid param to load text \n");
		return -1;
	}
    ...
    if (ret < sizeof(res_head) || res_head.version != 2)
    {
        ...
        os_printk("error: unsupported encoding %d\n", res_head.version);
        return -1;
    }
    ...
    {
    	os_printk("error: cannot find string resource in %s, id %d\n", info->str_path, text->sty_data->id);
    	return -1;
    }
...
//	SYS_LOG_INF("offset %d, length %d", res_entry.offset, res_entry.length);
	...
	if(text->buffer == NULL)
	{
		os_printk("error: cannot get text buffer for %s : id %d, offset %d, length %d\n", info->str_path, text->sty_data->id, res_entry.offset, res_entry.length);
		return -1;
	}
...
//	SYS_LOG_INF("text buffer 0x%x\n", text->buffer);
	...
	{
		os_printk("error: cannot read string in %s for resource id %d, offset %d, length %d\n", info->str_path, text->sty_data->id, res_entry.offset, res_entry.length);
		return -1;
	}
...
//	SYS_LOG_INF("resource->type 0x%x", resource.type);
    if(resource->type == RESOURCE_TYPE_GROUP)
    {
    	...
    	if(res == NULL)
    	{
    		os_printk("%s %d: error: no space for resource \n", __func__, __LINE__);
    		return NULL;
    	}
...
//		SYS_LOG_INF("pic w %d, h %d, id 0x%x", res->width, res->height, res->id);
		...
		if(ret < 0)
		{
			os_printk("%s %d: get scene child bitmap faild\n", __func__, __LINE__);
...
//    	SYS_LOG_INF("\n ### resource id 0x%x\n", resource->id);
    ...
    {
    	os_printk("%s %d: cant find resource 0x%x \n", __func__, __LINE__, id);
    	return NULL;
    }
...
//		SYS_LOG_INF("pic w %d, h %d, id 0x%x", res->width, res->height, res->id);
		...
		if(ret < 0)
		{
			os_printk("%s %d: get group child bitmap faild\n", __func__, __LINE__);
			res_manager_release_resource(res);
			res = NULL;
		}
		return res;
...
resource_bitmap_t* res_manager_load_frame_from_picregion(resource_info_t* info, resource_picregion_t* picreg, uint32_t frame)
{
	...
	if(bitmap == NULL)
	{
        os_printk("no space for frame loading \n");
        return NULL;
	}
	...
	if(ret < 0)
	{
		os_printk("picregion load frame faild \n");
...
int32_t res_manager_preload_bitmap(resource_info_t* res_info, resource_bitmap_t* bitmap)
{
	...
	if(ret < 0)
	{
		os_printk("preload bitmap error 0x%x, %p\n", bitmap->sty_data->id, bitmap->sty_data);
		return -1;
	}
	else
	{
...
//	SYS_LOG_INF("\n res_manager_preload_bitmap_compact scene_id 0x%x\n", scene_id);
	...
	if(ret < 0)
	{
		os_printk("preload bitmap compact error scene 0x%x, 0x%x\n", scene_id, bitmap->sty_data->id);
		return -1;
	}
	else
	{
...
int32_t res_manager_load_bitmap(resource_info_t* res_info, resource_bitmap_t* bitmap)
{
	...
	if(ret < 0)
	{
		os_printk("load bitmap error 0x%x\n", bitmap->sty_data->sty_id);
		return -1;
	}
	else
	{
...
int32_t res_manager_init_compact_buffer(uint32_t scene_id, size_t size)
{
	...
	if(ret == NULL)
	{
		os_printk("alloc compact buffer failed: 0x%x, %d\n", scene_id, size);
		os_mutex_unlock(&bitmap_cache_mutex);
		return -1;
	}
...
void* res_manager_preload_from_scene(resource_info_t* res_info, resource_scene_t* scene, uint32_t id)
{
	...
	if ( scene == NULL )
	{
		os_printk("%s %d: null scene to preload \n", __func__, __LINE__);
		return NULL;
	}
...
//		SYS_LOG_INF("resource id 0x%x, offset 0x%x",resource.id, resource.offset);
	...
	if(i >= scene->resource_sum)
	{
		os_printk("%s %d: error: cannot find resource 0x%x in scene 0x%x\n", __func__, __LINE__, id, scene->scene_id);
		return NULL;
	}
...
//	os_printk("wrong type to preload");
	return NULL;
}
...
//	SYS_LOG_INF("resource->type 0x%x", resource.type);
	if(resource->type == RESOURCE_TYPE_PICTURE)
	{
		...
		if(res == NULL)
		{
			os_printk("%s %d: error: no space for resource to preload\n", __func__, __LINE__);
			return NULL;
		}
		memset(res, 0, sizeof(resource_bitmap_t));
		sty_picture_t* pic = (sty_picture_t*)resource;
...
resource_bitmap_t* res_manager_preload_from_picregion(resource_info_t* res_info, resource_picregion_t* picreg, uint32_t frame)
{
	...
	{
        os_printk("invalid picreg for preload \n");
        return NULL;
	}
	...
	if(bitmap == NULL)
	{
        os_printk("no space for frame preloading \n");
        return NULL;
	}
...
void* res_manager_preload_next_group_child(resource_info_t * info, resource_group_t* group, int* count, uint32_t* offset, uint32_t scene_id, uint32_t pargroup_id)
{
	...
	if(group == NULL)
	{
		os_printk("invalid param for res_manager_preload_next_group_child\n");
		return NULL;
	}
...
void* res_manager_preload_next_scene_child(resource_info_t* info, resource_scene_t* scene, uint32_t* count, uint32_t* offset)
{
	...
	if(scene == NULL)
	{
		os_printk("invalid param for res_manager_preload_next_scene_child\n");
		return NULL;
	}
...
void res_manager_dump_info(void)
{
	...
	if(mem_peak > 0)
	{
		os_printk("res mem peak size is %d\n", mem_peak);
	}
	else
	{
		os_printk("res mem peak statistic not enabled\n");
	}

	//ui mem info
	os_printk("full screen bitmap total %d\n", ui_mem_total);

	//compact buffer dump();
	citem = bitmap_buffer.compact_buffer_list;
	while(citem != NULL)
	{
		os_printk("compact buffer sceneid 0x%x, addr %p, size %d, freesize %d \n", citem->scene_id, citem->addr, citem->offset, citem->free_size);
		citem = citem->next;
	}
	...
	while(bitem != NULL)
	{
		os_printk("bitmap buffer id %d, width %d, height %d, addr %p, size %d, ref %d \n", bitem->id, bitem->width, bitem->height, bitem->addr, bitem->size, bitem->ref);
		bitem = bitem->next;
	}
	...
	while(bitem != NULL)
	{
		os_printk("txt buffer id %d, addr %p \n", bitem->id, bitem->addr);
		bitem = bitem->next;
	}
...
void res_manager_compact_buffer_check(uint32_t scene_id)
{
	...
	while(citem != NULL)
	{
		...
		{
			os_printk("RES LEAK WARNING: scene 0x%x compact buffer exsit when layout scene 0x%x\n", citem->scene_id, scene_id);
		}
		citem = citem->next;
	}
