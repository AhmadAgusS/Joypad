...
static char playback_output_buffer[0x1000]   __in_section_unique(DSP_SHARE_RAM);  //16bit: 0x800, 32bit: 0x1000
#else
static char output_pcm[0x1000];
static char output_decoder[0x1000]				__in_section_unique(DSP_SHARE_RAM);  //16bit: 0x800, 32bit: 0x1000
static char playback_output_buffer[0x4000]   __in_section_unique(DSP_INNER_RAM);  //16bit: 0x800, 32bit: 0x1000
#endif
...
static const struct media_memory_block media_memory_config[] = {
	{
		...
		.mem_cell = {
            {.mem_type = OUTPUT_PKG_HDR,  .mem_base = (uint32_t)&playback_input_buffer[0x1800], .mem_size = 0x100,},
			{.mem_type = INPUT_PLAYBACK,  .mem_base = (uint32_t)&playback_input_buffer[0x1900], .mem_size = sizeof(playback_input_buffer)-0x100,},
	...
	},
	{
		...
		#endif
			{.mem_type = OUTPUT_PARSER,   .mem_base = (uint32_t)&playback_input_buffer[0], .mem_size = 0x2000,},
            {.mem_type = PARSER_EVT_BUFFER, .mem_base = (uint32_t)&playback_input_buffer[0x2000], .mem_size = 0x20,},
			{.mem_type = BT_TRANSMIT_INPUT,    .mem_base = (uint32_t)&playback_input_buffer[0x2020], .mem_size = 0xC00,},
			{.mem_type = BT_TRANSMIT_OUTPUT,   .mem_base = (uint32_t)&playback_input_buffer[0x2C20], .mem_size = 0x800,},
        #ifdef CONFIG_MEDIA_EFFECT
			{.mem_type = DAE_PARAM,  .mem_base = (uint32_t)&dae_para, .mem_size = sizeof(dae_para),},
        #endif
	...
	},
	{
		...
		.mem_cell = {
			{.mem_type = INPUT_PLAYBACK,  .mem_base = (uint32_t)&playback_input_buffer[0x000], .mem_size = 0x400,},
			{.mem_type = OUTPUT_DECODER,  .mem_base = (uint32_t)&playback_input_buffer[0x400], .mem_size = 0x2A8,},
			...
			{.mem_type = INPUT_CAPTURE,  .mem_base = (uint32_t)&playback_input_buffer[0xAA8], .mem_size = 0x200,},
			{.mem_type = INPUT_ENCBUF,   .mem_base = (uint32_t)&playback_input_buffer[0x12A8], .mem_size = 0x200,},
			{.mem_type = OUTPUT_CAPTURE, .mem_base = (uint32_t)&playback_input_buffer[0x15A8], .mem_size = 0x200,},
...

	{
		...
		.mem_cell = {
			{.mem_type = INPUT_PCM,      .mem_base = (u32_t)&playback_input_buffer[0x0000], .mem_size = 0x400,},
			{.mem_type = INPUT_CAPTURE,  .mem_base = (uint32_t)&playback_input_buffer[0x400], .mem_size = 0x400,},
			{.mem_type = INPUT_ENCBUF,   .mem_base = (u32_t)&playback_input_buffer[0x800], .mem_size = 0x800,},
			{.mem_type = OUTPUT_CAPTURE, .mem_base = (u32_t)&playback_input_buffer[0x1000], .mem_size = 0x800,},
		},
	},
