...
static void gc9c01_apply_brightness(struct gc9c01_data *data, uint8_t brightness)
{
	...
	if (data->has_refreshed && data->frame_refreshed)
			 ...
			 const void *buf)
{
	...
	/* check last line refreshed */
	if (y + desc->height >= gc9c01_videomode.vactive) {
		data->frame_refreshed = 1;
	}
...
#ifdef CONFIG_PANEL_TE_GPIO
	if (gc9c01_videomode.flags & (DISPLAY_FLAGS_TE_LOW | DISPLAY_FLAGS_TE_HIGH)) {
...
#else
	gc9c01_set_orientation(dev, DISPLAY_ORIENTATION_NORMAL);
#endif
...
static int gc9c01_init(const struct device *dev)
{
	...
	k_work_init(&data->timer, gc9c01_resume);
#endif
