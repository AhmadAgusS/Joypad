...
void display_pic(void *pic_addr, int pic_width, int pic_height, uint32_t bg_color)
{
	...
	uint16_t bytes_per_line = img_buf.desc.pitch * bits_per_pixel / 8;
	...
	for(int i = 0; i < LCD_HEIGHT;) {
		...
		display_buffer_fill_color(&dst_buf, color);
		draw_region.y1 = i;
		...
		if (ui_region_intersect(&copy_region, &draw_region, &img_region)) {
				...
				display_buffer_fill_image(&img_buf,(void *)((uint32_t)pic_addr + (copy_region.y1 - img_region.y1) * pic_width * bits_per_pixel / 8));
		}
