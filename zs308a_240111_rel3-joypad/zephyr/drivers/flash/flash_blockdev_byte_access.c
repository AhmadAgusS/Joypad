...
LOG_MODULE_REGISTER(spinand_flash_acts, CONFIG_FLASH_LOG_LEVEL);
...
static int  _block_dev_flush_cache(const struct device *dev, struct block_dev_cache_item *item)
{
	...
	if(item->write_valid && item->cache_valid){
		...
		err = flash_write(dev, item->cache_start_sector , item->cache_data, BLOCK_DEV_CACHE_SECT_NUM);
		if (err < 0) {
...
static int  _block_dev_update_cache(const struct device *dev, struct block_dev_cache_item *item, u32_t sec_off, u16_t *seq)
{
	...
	*seq = *seq + 1;
	...
	err = flash_read(dev, item->cache_start_sector ,item->cache_data, BLOCK_DEV_CACHE_SECT_NUM);
	if (err < 0) {
...
static void block_dev_storage_read_write(struct block_dev_flash_data *data, uint32_t addr,
...
static int block_dev_storage_write(const struct device *dev, off_t offset,
			      const void *data,
			      size_t len)
{
...
static int block_dev_storage_read(const struct device *dev, off_t offset,
			      void *data,
			      size_t len)
{
	struct block_dev_flash_data *dev_data = DEV_DATA(dev);
...
static int block_dev_storage_erase(const struct device *dev, off_t offset,
			       size_t size)
{
        ...
        case PM_DEVICE_ACTION_TURN_OFF:
            for(i = 0; i < BLOCK_DEV_CACHE_NUM_ITEM; i++) {
                if (data->item[i].write_valid && data->item[i].cache_valid) {
                    ...
                    err = flash_write(data->flash_dev, offset, buf, BLOCK_DEV_CACHE_SECT_NUM);
                    if (err < 0) {
                        LOG_ERR("nand write error %d, offsec 0x%x, buf %p.\n", err, offset, buf);
                        return -EIO;
                    }
...
DEVICE_DEFINE(block_dev_spinand_acts, CONFIG_BLOCK_DEV_FLASH_NAME, &block_dev_flash_init, block_dev_pm_control, &block_dev_flash_acts_data,
