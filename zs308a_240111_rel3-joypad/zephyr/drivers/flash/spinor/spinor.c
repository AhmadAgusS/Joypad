...
_nor_fun void spinor_write_cmd(struct spinor_info *sni, unsigned char cmd)
{
	spimem_write_cmd_addr(&sni->spi, cmd, 0, 0);	
}
...
_nor_fun static bool spinor_resume_and_check_idle(struct spinor_info *sni, unsigned int addr)
{
	bool ret;
	uint32_t key, i;
	char tmp[8];
...
_nor_fun static void spinor_wait_finished(struct spinor_info *sni, unsigned int addr)
{
	int i;
	uint32_t key;  
