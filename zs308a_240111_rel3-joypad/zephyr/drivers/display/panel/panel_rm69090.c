				 ...
				 const uint16_t y, const uint16_t w, const uint16_t h)
{
	...
	uint16_t ram_x = x + CONFIG_PANEL_OFFSET_X + 12;
			 ...
			 const void *buf)
{
	...
	res = display_controller_write_pixels(data->lcdc_dev,
			(0x32 << 24 | RM69090_CMD_RAMWR << 8), DC_INVALID_CMD, desc, buf);
	if (res) {
...
static void rm69090_start_de_transfer(void *arg)
{
	...
	res = display_controller_write_pixels(data->lcdc_dev,
			(0x32 << 24 | RM69090_CMD_RAMWR << 8), DC_INVALID_CMD,
			&data->refr_desc, NULL);
	if (res) {
		LOG_ERR("write pixels failed");
		data->transfering = 0;
...
static __unused void rm69090_exit_sleep(struct rm69090_data *data)
{
	rm69090_transmit_begin();
...
static int rm69090_pm_early_suspend(const struct device *dev)
{
	struct rm69090_data *data = (struct rm69090_data *)dev->data;
...
static int rm69090_pm_control(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_EARLY_SUSPEND:
		if (soc_get_aod_mode()) {
			...
			ret = rm69090_pm_early_suspend(dev);
		}
		break;
