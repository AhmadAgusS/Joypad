...
static void st7789v2_Clear(struct st7789v2_data *data, uint16_t color)
{
	...
	st7789v2_transmit(data, ST7789V2_CMD_RAMWR, 0, 0);
	 ...
	 for (i = 0; i < 240 * 320; i++) {
		display_controller_write_pixels(data->lcdc_dev, DC_INVALID_CMD, DC_INVALID_CMD, &desc, &color);
	 }
...
static void st7789v2_Clear_dbg(struct st7789v2_data *data, uint16_t color)
{
    ...
    display_controller_write_pixels(data->lcdc_dev, DC_INVALID_CMD, DC_INVALID_CMD, &desc, dis_buf);
			 ...
			 const void *buf)
{
	...
	return display_controller_write_pixels(data->lcdc_dev, DC_INVALID_CMD, DC_INVALID_CMD, desc, buf);
}
...
static void st7789v2_start_transfer(void *arg)
{
	...
	/* source from DE */
	//display_controller_write_pixels(data->lcdc_dev, (0x32 << 24 | 0x2c << 8), DC_INVALID_CMD,
			//&data->refr_desc, NULL);
}
...
#ifdef CONFIG_LCD_WORK_QUEUE
			k_delayed_work_submit_to_queue(&data->lcd_work_q, &data->timer, K_NO_WAIT);
#else
