...
#  define CONFIG_PANEL_BACKLIGHT_DELAYED_FRAMES 2
#endif
#endif
				 ...
				 const uint16_t y, const uint16_t w, const uint16_t h)
{
	...
	uint16_t ram_x = x + CONFIG_PANEL_OFFSET_X;
	uint16_t ram_y = y + CONFIG_PANEL_OFFSET_Y;
			 ...
			 const void *buf)
{
	...
	res = display_controller_write_pixels(data->lcdc_dev,
			(0x32 << 24 | GC9C01_CMD_RAMWR << 8), DC_INVALID_CMD, desc, buf);
	if (res) {
...
static void gc9c01_start_de_transfer(void *arg)
{
	...
	res = display_controller_write_pixels(data->lcdc_dev,
			(0x32 << 24 | GC9C01_CMD_RAMWR << 8), DC_INVALID_CMD,
			&data->refr_desc, NULL);
	if (res) {
		LOG_ERR("write pixels failed");
		data->transfering = 0;
...
static __unused void gc9c01_exit_sleep(struct gc9c01_data *data)
{
...
static int gc9c01_pm_early_suspend(const struct device *dev)
{
	struct gc9c01_data *data = (struct gc9c01_data *)dev->data;
...
static int gc9c01_pm_control(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_EARLY_SUSPEND:
		ret = gc9c01_pm_early_suspend(dev);
		break;
