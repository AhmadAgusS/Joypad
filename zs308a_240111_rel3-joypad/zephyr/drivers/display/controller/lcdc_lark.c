...
static int lcdc_config_port(const struct device *dev, const struct display_videoport *port)
{
	...
	if (port->type != DISPLAY_PORT_SPI_QUAD_SYNC) {
		lcd_ctl |= LCD_HOLD_OPT;
	}
	...
	case DISPLAY_PORT_SPI:
		...
		LCDC->SPI_CTL = LCD_SPI_CDX(1) | LCD_SPI_AHB_CSX(1) | LCD_SPI_AHB_F565_16BIT |
				LCD_SPI_TYPE_SEL(port->minor_type == LCD_SPI_QUAD_SYNC ? LCD_SPI_QUAD : port->minor_type) |
				LCD_SPI_SCLK_POL((port->spi_mode.cpol == port->spi_mode.cpha) ? 0 : 1) |
				LCD_SPI_DELAY_CHAIN_SEL(port->spi_mode.rx_delay_chain_ns) |
				LCD_SPI_RDLC_SEL(port->spi_mode.rx_dummy_cycles) |
...
static int lcdc_config_mode(const struct device *dev, const struct display_videomode *mode)
{
	...
	if (data->port->type == DISPLAY_PORT_SPI_QUAD_SYNC) {
		uint16_t data_cycles = mode->hactive *
			display_format_get_bits_per_pixel(mode->pixel_format) / 4;

		LCDC->QSPI_DTAS = (0 << 16) | data_cycles;
		LCDC->QSPI_SYNC_TIM = LCD_QSI_QSYNC_TIMING(
		...
		enum display_controller_source_type source_type)
{
	...
	case DISPLAY_PORT_SPI:
		...
		switch (source_type) {
		case DISPLAY_CONTROLLER_SOURCE_ENGINE:
			LCDC->SPI_CTL |= LCD_SPI_INPUT_SRC_SEL_DE | LCD_SPI_FTC_IRQ_EN | LCD_SPI_FEND_PD_EN | LCD_SPI_CDX(1);
			break;
		case DISPLAY_CONTROLLER_SOURCE_DMA:
			LCDC->SPI_CTL |= LCD_SPI_INPUT_SRC_SEL_DMA | LCD_SPI_FTC_IRQ_EN | LCD_SPI_FEND_PD_EN | LCD_SPI_CDX(1);
			break;
		case DISPLAY_CONTROLLER_SOURCE_MCU:
...
#define SPI_AHB_DATA_MASK (SPI_AHB_CMD_MASK | LCD_SPI_AHB_F565_MASK | LCD_SPI_AHB_CFG_DATA | LCD_SPI_RWL_MASK)
		...
		uint32_t cmd, void *buf, uint32_t len)
{
	...
	if (lcdc_data->port->major_type == DISPLAY_PORT_SPI) {
		...
		if (lcdc_data->port->minor_type < DISPLAY_SPI_QUAD) {
			...
			if (len <= 4) {
				LCDC->SPI_CTL |= LCD_SPI_INPUT_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA | LCD_SPI_RWL(len);

				uint32_t data = LCDC->DATA;
				_read_config_data32(buf, &data, len);
			} else {
				LCDC->SPI_CTL |= LCD_SPI_INPUT_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA | LCD_SPI_RWL(1);
				do {
			...
			LCDC->SPI_CTL |= LCD_SPI_QUAD | LCD_SPI_INPUT_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA |
...
static int _fill_config_data32(uint32_t *data, const uint8_t *buf8, uint32_t len)
{
	...
	if (lcdc_data->port->major_type == DISPLAY_PORT_SPI) {
		...
		if (lcdc_data->port->minor_type < DISPLAY_SPI_QUAD) {
			...
			if (len > 0) {
				...
				LCDC->SPI_CTL &= ~(SPI_AHB_DATA_MASK | LCD_SPI_DUAL_LANE_MASK);
				if (len <= 4) {
					LCDC->SPI_CTL |= LCD_SPI_INPUT_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA |
							LCD_SPI_CDX(1) | LCD_SPI_RWL(len);

					_fill_config_data32(&data, buf8, len);
					LCDC->DATA = data;
				} else {
					LCDC->SPI_CTL |= LCD_SPI_INPUT_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA |
			...
			/* config spi type 'LCD_SPI_QUAD_SYNC' to 'LCD_SPI_QUAD' temporarily */
			LCDC->SPI_CTL &= ~(SPI_AHB_DATA_MASK | LCD_SPI_TYPE_MASK);
			LCDC->SPI_CTL |= LCD_SPI_QUAD | LCD_SPI_INPUT_SRC_SEL_AHB | LCD_SPI_AHB_CFG_DATA |
					LCD_SPI_CDX(0) | LCD_SPI_RWL(len);
			 ...
			 */
			i = _fill_config_data32(tmp_data, buf8, len);
		...
		const struct display_buffer_descriptor *desc)
{
	...
	case DISPLAY_PORT_SPI:
		...
		if (data->port->minor_type == DISPLAY_SPI_QUAD_SYNC) {
			LCDC->QSPI_CMD1 = hsync_cmd; /* only required for QUAD_SYNC */
			LCDC->QSPI_IMG_SIZE = LCD_QSI_IMG_SIZE(desc->width, desc->height);

			LCDC->CTL &= ~LCD_EN;
			/* set to quad sync mode */
			LCDC->SPI_CTL = (LCDC->SPI_CTL & ~LCD_SPI_TYPE_MASK) | LCD_SPI_QUAD_SYNC;
			LCDC->CTL |= LCD_EN;
		}
		...
		const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	bytes_per_line = desc->pitch * bytes_per_pixel;
