...
uint8_t current_init_charge_stage1_NTC[5] = {0, 20, 30, 30, 0};
uint8_t current_init_charge_stage2_NTC[5] = {0, 50, 70, 60, 0};
uint8_t current_init_charge_stage3_NTC[5] = {0, 80, 200, 100, 0};
#endif
...
void bat_adc_sample_buf_init(void)
{
    ...
    while(1) {
		try_cnt++;
		bat_adc_value = bat_adc_get_sample();
		volt_mv = bat_adc_get_voltage_mv((uint32_t)bat_adc_value);
		if((volt_mv >= BAT_VOLTAGE_SAMPLE_VALID_MIN) && (volt_mv <= BAT_VOLTAGE_SAMPLE_VALID_MAX)) {
        	bat_charge->bat_adc_sample_buf[i] = (uint16_t)bat_adc_value;
			k_usleep(2000); //delay 2ms
			valid_sv_cnt++;
			i++;
		} else {
		...
		if(valid_sv_cnt >= buf_size) {
			LOG_INF("fill batadc buf ok:%d\n\n", valid_sv_cnt);
			break;
		} else if(try_cnt > 200) { //400ms+ timeout
			LOG_INF("fill batadc timeout, valid:%d\n\n", valid_sv_cnt);
			break;			
		} else {
			; //empty
		}
...
#if 0
	if(configs->cfg_charge.Init_Charge_Enable == NO) {
		LOG_INF("init charge is not enabled!");
		return;
	}
#endif
