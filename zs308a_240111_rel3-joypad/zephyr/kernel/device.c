...
void z_sys_init_run_level(int32_t level)
{
    ...
    for (entry = levels[level]; entry < levels[level+1]; entry++) {
        ...
        if(dev){
            printk("z_sys_init(%d) entry(%p) func:%p, grp_end=%p, dev(%s) \n", level, entry, entry->init, levels[level+1], dev->name);
        }else{
            printk("z_sys_init(%d) entry(%p) func:%p, grp_end=%p\n", level, entry, entry->init, levels[level+1]);
        }
#endif
