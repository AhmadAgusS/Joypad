...
static int command_handler(const struct device *dev, char *str, int len)
{
	/* end with ENTER */
	if (str[len - 1] != 0x0D) {
		printk("0x%x\n", str[len - 1]);
...
static int read_and_echo_data(const struct device *dev, int *bytes_read)
{
	...
	while (cdc_acm_enabled) {
		if (cdc_acm_perf == 0) {
			...
			if (++count == perf_count) {
				...
				cdc_acm_perf = 0;
				LOG_WRN("write ms: %d, speed: %dKB/s\n", ms,
