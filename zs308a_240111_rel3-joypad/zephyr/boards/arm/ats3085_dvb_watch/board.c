...
static const struct acts_pin_config pre_board_pin_config[] = {
	/*UART0 */
...
static const struct acts_pin_config board_pin_config[] = {
	...
	/* uart2 tx */
	PIN_MFP_SET(GPIO_53,  UART2_MFP_CFG),
	/* uart2 rx */
	PIN_MFP_SET(GPIO_54,  UART2_MFP_CFG),
#endif
...
static int board_pre_early_init(const struct device *arg)
{
	ARG_UNUSED(arg);

	acts_pinmux_setup_pins(pre_board_pin_config, ARRAY_SIZE(pre_board_pin_config));
	return 0;
}
...
SYS_INIT(board_pre_early_init, PRE_KERNEL_1, 5);
SYS_INIT(board_early_init, PRE_KERNEL_2, 32);
