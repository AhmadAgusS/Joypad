...
#ifndef CONFIG_DVFS_DYNAMIC_LEVEL
...
static inline int dsp_soc_request_addr(int cpu_addr)
{
	if (cpu_addr >= CPU_EXT_BASE && cpu_addr < CPU_EXT_BASE + EXT_MEM_SIZE) {
		...
		sys_write32(sys_read32(CMU_MEMCLKSRC0)  | (1 << 20), CMU_MEMCLKSRC0);
#else
...
{
#ifdef CONFIG_SOC_NO_PSRAM
	sys_write32((sys_read32(CMU_MEMCLKSRC0) | (1 << 20) | (1 << 12)),CMU_MEMCLKSRC0);
#else
