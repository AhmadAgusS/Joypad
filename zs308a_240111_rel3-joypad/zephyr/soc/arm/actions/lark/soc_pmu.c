...
{
	unsigned int sel, vctr1, vdd_ldo;

	if (volt_mv < 550 || volt_mv > 1300)
		return ;
	vctr1 = sys_read32(VOUT_CTL1_S1);
	vdd_ldo = !(vctr1 & (1<<30));  // VDD LDO mode
	if(vdd_ldo){
		if(volt_mv >= 1200){
				/* vd12 set as 1.25v */
			sys_write32((vctr1 & ~(0xff << 8)) | (0xb << 8) | (0xb << 12), VOUT_CTL1_S1);
			k_busy_wait(1);
		}
	}
...
int soc_pmu_get_temperature(void)
{
	...
	/* sample temerature formula: temperature = 4654 - (adc_temp x 1745 / 1000);
	 * precision: 0.1degree
	 * sample result multiply 10
	 */
	ret = 4654 - (1745 * adc_temp / 1000);
