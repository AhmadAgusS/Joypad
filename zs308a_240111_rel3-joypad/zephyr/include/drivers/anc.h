 ...
 */
typedef int (*anc_api_poweron)(const struct device *dev);

/**
 * @typedef anc_api_poweroff
 * @brief Callback API for power off
 */
typedef int (*anc_api_poweroff)(const struct device *dev);
 ...
 */
typedef int (*anc_api_request_image)(const struct device *dev, const struct anc_imageinfo *image);

/**
 * @typedef anc_api_release_image
 * @brief Callback API for releasing image
 */
typedef int (*anc_api_release_image)(const struct device *dev);

/**
 * @typedef anc_api_release_mem
 * @brief Callback API for releasing image
 */
typedef int (*anc_api_request_mem)(const struct device *dev, int type);

/**
 * @typedef anc_api_release_mem
 * @brief Callback API for releasing image
 */
typedef int (*anc_api_release_mem)(const struct device *dev, int type);

/**
 * @typedef anc_api_get_status
 * @brief Callback API for get anc status
 */
typedef int (*anc_api_get_status)(const struct device *dev);

/**
 * @typedef anc_api_send_command
 * @brief Callback API for send command to anc dsp
 */
typedef int (*anc_api_send_command)(const struct device *dev, int type, void *data, int size);
 ...
 */
static inline int anc_poweron(const struct device *dev)
{
 ...
 * @retval Negative errno code if failure.
 */
static inline int anc_poweroff(const struct device *dev)
{
	const struct anc_driver_api *api = dev->api;
 ...
 * @retval Negative errno code if failure.
 */
static inline int anc_request_image(const struct device *dev, const struct anc_imageinfo *image)
{
    const struct anc_driver_api *api = dev->api;
 ...
 * @retval Negative errno code if failure.
 */
static inline int anc_release_image(const struct device *dev)
{
    const struct anc_driver_api *api = dev->api;
 ...
 * @retval Negative errno code if failure.
 */
static inline int anc_request_mem(const struct device *dev,int type)
{
    const struct anc_driver_api *api = dev->api;
 ...
 * @retval Negative errno code if failure.
 */
static inline int anc_release_mem(const struct device *dev, int type)
{
    const struct anc_driver_api *api = dev->api;
 ...
 */
static inline int anc_get_status(const struct device *dev)
{
	const struct anc_driver_api *api = dev->api;
 ...
 */
static inline int anc_send_command(const struct device *dev, int type, void *data, int size)
{
	const struct anc_driver_api *api = dev->api;
 ...
 * @retval -1 failed
 */
static inline int anc_fs_change(const struct device *dev, uint32_t samplerate, uint32_t dac_digctl)
{
	uint32_t data[2];
	const struct anc_driver_api *api = dev->api;
