...
#if IS_ENABLED(CONFIG_I2C_0_USE_DMA) || IS_ENABLED(CONFIG_I2C_1_USE_DMA)
#define I2C_DMA_ENABLED			// if I2C_0/I2C1 use dma to transfer, force to declare tx_buf and rx_buf
#endif

#if defined(CONFIG_I2C_ASYNC_MSG_INTERNAL_BUFFER) || defined(I2C_DMA_ENABLED)
#define I2C_MSG_ASYNC_RX_MAX_BUF (64)	// 64 bytes data
#define I2C_MSG_ASYNC_TX_MAX_BUF (65)	// 1 byte(slave address+R/W bit) + 64 bytes data
#endif
...
#if (defined(CONFIG_I2C_ASYNC) && defined(CONFIG_I2C_ASYNC_MSG_INTERNAL_BUFFER)) || defined(I2C_DMA_ENABLED)
 ...
 * @retval -EIO General input / output error.
 *
 * @note In case of not define CONFIG_I2C_ASYNC_MSG_INTERNAL_BUFFER(using cpu),  the parameter @a buf
 *		shall be a global address. But if user would like to use dma, we'll definitedly use internal buffer.
 */
