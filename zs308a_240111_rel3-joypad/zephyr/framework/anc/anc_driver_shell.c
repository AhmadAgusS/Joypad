...
{	
	...
	ringbuf = mcu_to_anc_address(POINTER_TO_UINT(dump_ringbuf), 0);
...

{
	...
	const struct device *anc_dev = device_get_binding(CONFIG_ANC_NAME);
	const struct device *aout_dev = device_get_binding(CONFIG_AUDIO_OUT_ACTS_DEV_NAME);
