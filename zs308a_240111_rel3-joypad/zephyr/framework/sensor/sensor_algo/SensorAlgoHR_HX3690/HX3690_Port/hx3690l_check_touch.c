...
hx3690_hrs_wear_msg_code_t hx3690l_wear_status = MSG_HRS_NO_WEAR;
hx3690_hrs_wear_msg_code_t hx3690l_wear_status_pre = MSG_HRS_NO_WEAR;
...
void hx3690l_check_touch_init(void) //20200615 ericy ppg fs=25hz, phase3 conversion ready interupt en
{
    ...
    uint8_t phase3_ldr_sel = 1;     /* 1 = ldr1; 2 = ldr2; 4 = ldr3; 8 = ldr4 ; */
...
/////////FIFO and adc conversion ready config////////
  ...
  #if defined(INT_MODE)
    hx3690l_write_reg(0X13, 0x10);   /* bits<7:4> fifo data sel, 0000 = p1-p2;0001= p1,p2;0010=p3,p4;
...
bool hx3690_check_touch_open(void)
{
    return true;
}
...
{ 
  uint8_t ii;
  hx3690_hrs_wear_msg_code_t wear_status = MSG_HRS_NO_WEAR;
  if(count==0)
  {
    return wear_status;
  }
...
hx3690_hrs_wear_msg_code_t check_touch_alg(int32_t ir_data)
{
   if(ir_data>check_mode_wear_thre)
   {
     ...
     if(touch_cnt>=24)
     {
        hx3690l_wear_status = MSG_HRS_WEAR;
        touch_cnt=0;
     }
     else
     {
   ...
   else if(ir_data<check_mode_unwear_thre)
   {
     ...
     if(notouch_cnt>=24)
     {
        hx3690l_wear_status = MSG_HRS_NO_WEAR;
        notouch_cnt=0;
     }
     else
     {
   ...
   if(hx3690l_wear_status != hx3690l_wear_status_pre)
   {
     if(hx3690l_wear_status==MSG_HRS_WEAR)
     {
       //DEBUG_PRINTF(0,"11111===>>> hx3690l_wear_status = %d\r\n" ,hx3690l_wear_status);
         hx3690l_write_reg(0X31, 0x40);
         hx3690l_write_reg(0X1f, 0X2f);
     }
     else
     {
       //DEBUG_PRINTF(0,"22222===>>> hx3690l_wear_status = %d\r\n" ,hx3690l_wear_status);
         hx3690l_write_reg(0X31, 0x00);
         hx3690l_write_reg(0X1f, 0X29);
     }
     hx3690l_wear_status_pre = hx3690l_wear_status;
   }
   return hx3690l_wear_status;
}
