...
//	os_api->dbgOutput("hx3605_delay_us %d \r\n",us);
}
...
//	os_api->dbgOutput("hx3605_delay %d \r\n",ms);
}
...
bool hx3605_write_reg(uint8_t addr, uint8_t data) 
{
	...
	if (os_api->i2c_write != NULL) {
		ret = os_api->i2c_write(5, addr, &data, 1);
	}
    return ret; 
}
...
bool hx3605_brust_read_reg(uint8_t addr , uint8_t *buf, uint8_t length) 
{
	...
	if (os_api->i2c_read != NULL) {
		ret = os_api->i2c_read(5, addr, buf, length);
	}
	
    return ret;      
}

// ¶¨Ê±Æ÷µ÷ÓÃº¯Êý void ppg_timeout_handler(void * p_context)
...
//	os_api->dbgOutput("320ms timer %d \r\n",en);
}
// agc µ÷ÓÃº¯Êý void agc_timeout_handler(void * p_context)
    ...
    bp_results_t    bp_alg_results ;   
      ...
      *count = 8;
      ...
      for(ii=0;ii<*count;ii++)
      {
			...
			{
					tyhx_hrs_alg_send_data(PPG_buf,*count, *als, gsen_fifo_x_send, gsen_fifo_y_send, gsen_fifo_z_send);  
			}				
    #endif      
