...
//constants
/******************************************************************************/
#define DBG(...)			if (cwm_os_api.dbgOutput) cwm_os_api.dbgOutput(__VA_ARGS__)
//#define DBG(...)			do {} while (0)

#define PI						(3.14159265f)

//#define CONFIG_SENSOR_ALGO_MOTION_CYWEE_DML 
...
int CWM_OS_i2cRead(uint16_t slaveAddr, uint16_t reg, int regLength, uint8_t *readData, int readDataSize, int busIndex) {
    return sensor_os_api.i2c_read(0, reg, readData, readDataSize);
}

int CWM_OS_i2cWrite(uint16_t slaveAddr, uint16_t reg, int regLength, uint8_t *writeData, int writeDataSize, int busIndex) {
    return sensor_os_api.i2c_write(0, reg, writeData, writeDataSize);
}
...
int sensor_algo_init(const sensor_os_api_t *api)
{
  ...
  memset(_heap_start, 0, (_heap_end - _heap_start));
	...
	// init code section
	if (_code_rom_end > _code_rom_start) {
		memcpy(_code_ram_start, _code_rom_start, (_code_rom_end - _code_rom_start));
	}
	...
	DBG("version:%d.%d.%d.%d product:%d model:%d\n", 
	...
	DBG("dml version:%d.%d.%d.%d product:%d model:%d\n", scl.iData[1], scl.iData[2], 
	...
	memset(&scl, 0, sizeof(scl));
    scl.iData[0] = 1;
    scl.iData[3] = 2;	//1:polling mode, 2:fifo mode
    scl.iData[4] = 1;	//1:any time
    ...
    for (int i = 0; i < scl.iData[2]; i ++) {

        DBG("DML device[%d]: hw_id=%d hw_attr=%d\n", i, scl.iData[6 + i * 2], scl.iData[7 + i * 2]);
    }
...
int sensor_algo_set_os_api(const sensor_os_api_t *api)
{
	// config os api
	sensor_os_api = *api;
	cwm_os_api.malloc = malloc;
	cwm_os_api.free = free;
	cwm_os_api.GetTimeNs = api->get_timestamp_ns;
	cwm_os_api.dbgOutput = api->dbgOutput;
#ifdef CONFIG_SENSOR_ALGO_MOTION_CYWEE_DML
	cwm_os_api.i2cRead = CWM_OS_i2cRead;
	cwm_os_api.i2cWrite = CWM_OS_i2cWrite;
#endif
