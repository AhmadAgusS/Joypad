...
static int read_otp_from_i2c(short *out)
{
	...
	buf[1] = 0x66;
	buf[2] = 0x9C;
	status = sensor_bus_write(&sensor_dev[ID_BARO], 0xC595, buf, 3);
	if (status)
		return status;
	
	// Read OTP values
	for (i = 0; i < 4; i++) {
		status = sensor_bus_read(&sensor_dev[ID_BARO], 0xC7F7, buf, 3);
		if (status)
			return status;
		out[i] = (buf[0] << 8) | buf[1];
	}
...
int icp10125_cvt(float *val, uint8_t *buf, uint16_t len)
{
	inv_invpres_t *s = &invpres;
	int press;
	int temp;
