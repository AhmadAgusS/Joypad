...
int sensor_hal_get_value(int id, sensor_dat_t *dat, uint16_t idx, float *val)
{
	...
	// convert data
	return sensor_hal_cvt_data(id, dat->buf + dat->sz * idx, dat->sz, val);
}

int sensor_hal_cvt_data(int id, uint8_t *buf, uint16_t len, float *val)
{
	// convert data
	return sensor_dev_cvt_data(&sensor_dev[id], buf, len, val);
}

void sensor_hal_config_tm(int id, int ms)
{
	// config timer period
	sensor_io_config_tm(&sensor_dev[id], ms);
}

uint32_t sensor_hal_get_tm(int id)
{
	// read timer cycle
	return sensor_io_get_tm(&sensor_dev[id]);
}
