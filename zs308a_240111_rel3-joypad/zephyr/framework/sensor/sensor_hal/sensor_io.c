...
void sensor_io_enable_trig(const sensor_dev_t *dev, int id)
{
    ...
    if (dev->io.int_io > 0) {
        ...
        if (task_trig->trig >= TIMER3 && task_trig->trig <= TIMER4) {
            sensor_tm_enable_irq(task_trig->trig, task_trig->peri, id, 0);
        }
    }
}
...
{
    task_trig_t *task_trig = (task_trig_t*)dev->task;
    int irq = !task_trig->en && (dev->io.int_io == 0);
...
void sensor_io_clear_tm_pending(int tid)
{
    ...
    acts_clock_peripheral_disable(CLOCK_ID_TIMER); //clk disable 
    sys_write32(0x2, CMU_TIMERCLK(tid)); // select RC4M
...
static int sensor_io_dev_init(const struct device *dev)
{
    ...
    if (!gpio_dev[3]) {
        //SYS_LOG_ERR("[gpiod] get device failed!");
        //return -1;
    }
