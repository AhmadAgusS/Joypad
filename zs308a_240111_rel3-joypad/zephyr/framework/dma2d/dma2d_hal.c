...
int hal_dma2d_rotation_start(hal_dma2d_handle_t *hdma2d, uint32_t src_address, uint32_t dst_address, uint16_t start_line, uint16_t num_lines)
{
	...
	if (dma2d_dev == NULL || hdma2d->instance < 0) {
		return -ENODEV;
	}
	...
	if (start_line + num_lines > hw_cfg->outer_diameter + 1) {
		return -EINVAL;
	}
	...
	hw_cfg->src_pitch = (hw_cfg->outer_diameter + 1 + rot_cfg->input_offset) * bytes_per_pixel;
	hw_cfg->dst_pitch = (hw_cfg->outer_diameter + 1 + ouput_cfg->output_offset) * bytes_per_pixel;
	...
	/* if (start_line == 0 || start_line != hw_cfg->line_end) */ {
		hw_cfg->dst_dist_sq = hw_cfg->outer_diameter * hw_cfg->outer_diameter +
				(hw_cfg->outer_diameter - 2 * start_line) * (hw_cfg->outer_diameter - 2 * start_line);
...
int hal_dma2d_config_rotation(hal_dma2d_handle_t *hdma2d)
{
	...
	hw_cfg->outer_radius_sq = cfg->outer_diameter * cfg->outer_diameter;
	hw_cfg->inner_radius_sq = cfg->inner_diameter * cfg->inner_diameter;
	hw_cfg->fill_color.full = cfg->fill_color;
	hw_cfg->fill_enable = cfg->fill_enable;
	hw_cfg->line_start = 0;
	hw_cfg->line_end = 0;

	center = FIXEDPOINT12(hw_cfg->outer_diameter) / 2 + PX_FIXEDPOINT12(0);
