...
static void draw_letter_cb(lv_disp_drv_t * disp_drv, lv_coord_t pos_x, lv_coord_t pos_y, lv_font_glyph_dsc_t * g,
...
void text_canvas_clean(lv_obj_t * obj)
{
	...
	gui_text_cache_free((void *)canvas->dsc.data);
...
static void text_canvas_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
	...
	if (code == LV_EVENT_STYLE_CHANGED) {
		...
		p->x = canvas->dsc_w;
		p->y = canvas->dsc.header.h - canvas->ext_draw_size;
...
static void text_canvas_draw(lv_event_t * e)
{
	...
	for (i = 0; i < parts; i++) {
		...
		if (_lv_area_intersect(&draw_area, &txt_area, clip_area) == false)
			continue;

		assert(((txt_area.x1 - draw_area.x1) & canvas->dsc_w_mask) == 0);

		lvgl_draw_img_dsc(&txt_area, &draw_area, &canvas->dsc, &draw_dsc);
		...
		if (canvas->emoji_dsc.count > 0) {
			...
			for (int i = canvas->emoji_dsc.count - 1; i >= 0; i--) {
				...
				lvgl_draw_img_dsc(&emoji_area, &draw_area, &emoji->img, &draw_dsc);
			}
		}
	}
}
...
static void text_canvas_refr_text(lv_obj_t *obj)
{
	...
	canvas->dsc.header.h = LV_MIN(canvas->dsc.header.h + canvas->ext_draw_size, IMG_MAX_H);

	uint32_t buf_size = (uint32_t)canvas->dsc.header.w * canvas->dsc.header.h * px_size / 8;
	if (canvas->dsc.data_size < buf_size) {
		gui_text_cache_free((void *)canvas->dsc.data);
		canvas->dsc.data = gui_text_cache_malloc(buf_size);
	...
	if (skip_txt_height) {
		...
		uint16_t dsc_h = feedback.line_count * line_height_font +
				(feedback.line_count - 1) * (draw_dsc.line_space) + canvas->ext_draw_size;
...
static uint32_t get_letter_on(lv_obj_t * obj, lv_draw_label_dsc_t *draw_dsc, lv_point_t * pos_in)
{
	...
	while (txt[line_start] != '\0') {
		new_line_start += _lv_txt_get_next_line(&txt[line_start],
				draw_dsc->font, draw_dsc->letter_space, max_w, draw_dsc->flag);
		...
		lv_img_dsc_t *img_dsc, lvgl_draw_label_result_t *feedback)
{
	...
	lv_disp_draw_buf_init(&draw_buf, (void *)img_dsc->data, NULL, img_dsc->header.w * img_dsc->header.h);
	lv_area_set(&draw_buf.area, 0, 0, img_dsc->header.w - 1, img_dsc->header.h - 1);

	lv_disp_drv_t driver;
	lv_disp_drv_init(&driver);
	driver.user_data = img_dsc;
	driver.draw_buf = &draw_buf;
	driver.hor_res = img_dsc->header.w;
	driver.ver_res = img_dsc->header.h;
	driver.set_px_cb = set_px_cb_alpha;
	driver.draw_letter_cb = draw_letter_cb;
	...
	if (canvas->emoji_dsc.max_count > 0) {
		lvgl_draw_label_with_emoji(&coords, &draw_buf.area, draw_dsc,
	...
	disp_ori = lv_obj_get_disp(obj);
	if (disp_ori->driver->gpu_fill_mask_cb) {
...
static lv_img_cf_t get_text_font_img_cf(const lv_font_t *font)
{
	...
	if (lv_font_get_glyph_dsc(font, &dsc_out, '1', '1') == false) {
		LV_LOG_WARN("fail to get font glyph bpp of '1', so fall back to 4\n");
						...
						const lv_area_t * clip_area, const uint8_t * map_p, lv_color_t color, lv_opa_t opa)
{
	...
	int16_t col_start = pos_x >= clip_area->x1 ? 0 : clip_area->x1 - pos_x;
	int16_t col_end   = pos_x + g->box_w <= clip_area->x2 + 1 ? g->box_w : clip_area->x2 - pos_x + 1;
	int16_t row_start = pos_y >= clip_area->y1 ? 0 : clip_area->y1 - pos_y;
	int16_t row_end   = pos_y + g->box_h <= clip_area->y2 + 1 ? g->box_h : clip_area->y2 - pos_y + 1;
	...
	lv_disp_draw_buf_t * vdb = disp_drv->draw_buf;
	uint16_t draw_row_ofs = lv_area_get_width(&vdb->area) * draw_bpp / 8;
	uint16_t draw_bit_col_ofs = (col_start + pos_x - vdb->area.x1) * draw_bpp;
	uint16_t draw_col_bit_init = draw_bit_col_ofs & 0x7;
	uint8_t *draw_p = (uint8_t *)vdb->buf_act +
		(row_start + pos_y - vdb->area.y1) * draw_row_ofs + (draw_bit_col_ofs >> 3);
