...
static void _alarm_set_btn_evt_handler(lv_event_t *e)
{
	...
	uint32_t time_sec = 0;
	lv_obj_t *par = p_alarm_set_data->obj_alarm_bg;
	alarm_sub_view_data_t *sview = p_alarm_set_data->sub_view;

	if (!p_alarm_set_data || !par)
		return;
...
static void _add_alarm(alarm_set_view_data_t *bg_view_data)
{
	int i = 0;
	uint32_t time_sec = 0;
	alarm_main_view_data_t *data = bg_view_data->main_view;

	if (!bg_view_data || !data)
		return;
...
static void _display_alarm_list_one(alarm_set_view_data_t *data, lv_obj_t *par, uint8_t index)
{
	...
	if (!data || !main_view || index >= ALARM_SET_CNT)
		return;
