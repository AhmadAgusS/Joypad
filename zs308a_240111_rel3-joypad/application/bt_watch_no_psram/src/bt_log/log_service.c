 ...
 *  STATIC PROTOTYPES
 **********************/
static void _sppble_connect_cb(int connected);
...
static int _tlv_unpack(io_stream_t stream, tlv_dsc_t *tlv)
{
	...
	return _tlv_unpack_data(stream, tlv);
}
...
static void _logsrv_actlog_send_syslog(logsrv_ctx_t *ctx, uint8_t syslog_type)
{
	...
	actlog_bt_transfer(syslog_type, _logsrv_actlog_traverse_callback);
...
static int _logsrc_proc_console_codes(logsrv_ctx_t *ctx, tlv_dsc_t *tlv)
{
	uint16_t code;
...
static int _logsrc_proc_tlv_cmd(logsrv_ctx_t *ctx, uint16_t param_len)
{
	uint8_t tlv_buf[4];
	tlv_dsc_t tlv = { .max_len = sizeof(tlv_buf), .buf = tlv_buf, };
	...
	while (param_len > 0) {
		...
		case TLV_TYPE_CONSOLE_CODE:
			if (_logsrc_proc_console_codes(ctx, &tlv))
				return -EINVAL;
			break;
...
static void _logsrv_proc_svc_cmd(logsrv_ctx_t *ctx)
{
	...
	switch (head.cmd) {
	case SVC_CMD_ID_RX:
		res = _logsrc_proc_tlv_cmd(ctx, param_len);
		break;
	default:
