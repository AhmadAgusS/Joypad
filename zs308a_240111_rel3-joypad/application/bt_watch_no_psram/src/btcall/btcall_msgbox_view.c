...
typedef struct btcall_view_data {
	...
	uint32_t ongoing_start_time;/*unit :s*/
...
static uint32_t _btcall_get_cur_time(void);
...
void btcall_set_ongoing_start_time(void)
{
	if (p_btcall_view_data && p_btcall_view_data->ongoing_start_time == 0) {
		p_btcall_view_data->ongoing_start_time = _btcall_get_cur_time();
	}
}
...
#if DEF_UI_WIDTH >= 454
	lvgl_bitmap_font_close(&data->font_28);
#endif
...
static int _display_ongoing_view(btcall_view_data_t *data)
{
	...
	if (data->cur_phone_number) {
		...
		_display_ongoing_label("calling", TXT_ONGOING_NUM);
	}

	/*dont display calling time for three way calling*/
	if (data->view_state != BTCALL_3W_STATE) {
		ongo_view->timer_ls = lv_timer_create(_display_ongoing_time_cb, 1000, data);
		if (data->ongoing_start_time == 0) {
			_display_ongoing_label("00:00", TXT_ONGOING_TIME);
			data->ongoing_start_time = _btcall_get_cur_time();
		} else {
...
{
	char time_buff[12] = { 0 };
	uint32_t second  = _btcall_get_cur_time() - p_btcall_view_data->ongoing_start_time;
...
static int _display_3way_view(btcall_view_data_t *data)
{
	...
	if (data->cur_phone_number) {
		...
		_display_3way_label("unkown", TXT_3W_HOLD_NUM);
	}

	if (data->thr_way_phone_number) {
		_display_3way_label(data->thr_way_phone_number, TXT_3W_ANOTH_NUM);
	} else {
		_display_3way_label("third number", TXT_3W_ANOTH_NUM);
	}
...
static int _display_incoming_view(btcall_view_data_t *data)
{
	...
	/* display coming info */
	_display_incoming_label("reject", TXT_REJECT);
	_display_incoming_label("accept", TXT_ACCEPT);
...
static void _btcall_view_layout(void)
{
	...
	if (p_btcall_view_data->calling_state == BTCALL_NULL_STATE) {
		...
		btcall_msgbox_popup_cb(BTCALL_MSGBOX_ID, MSG_MSGBOX_CLOSE, lv_scr_act() , p_btcall_view_data);
	} else {
...
//			msgbox_cache_popup(BTCALL_MSGBOX_ID, p_btcall_view_data);
			btcall_msgbox_popup_cb(BTCALL_MSGBOX_ID, MSG_MSGBOX_POPUP, lv_scr_act(), p_btcall_view_data);
		} else {
//			msgbox_cache_paint(BTCALL_MSGBOX_ID, 0);
			btcall_msgbox_popup_cb(BTCALL_MSGBOX_ID,MSG_MSGBOX_PAINT, lv_scr_act() , p_btcall_view_data);
		}
	}
...
static uint32_t _btcall_get_cur_time(void)
{
	return (uint32_t)(os_uptime_get() / 1000);
}
