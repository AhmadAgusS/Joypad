...
lv_obj_t * watch_list_add_btn(lv_obj_t *obj, const lv_img_dsc_t * icon_src, const lv_img_dsc_t * text_src)
{
	...
	if (icon_src) {
		...
		simple_img_set_src(obj_icon, icon_src);
	}

	if (text_src) {
		lv_obj_t *obj_text = simple_img_create(btn);
		simple_img_set_src(obj_text, text_src);
	}

	return btn;
}

lv_obj_t * watch_list_add_btn2(lv_obj_t *obj, const lv_img_dsc_t * icon_src, const char * text, lv_style_t *sty_text)
{
	watch_list_t *list = (watch_list_t *)obj;
	lv_obj_t *btn = lv_btn_create(obj);
	...
	if (text) {
		lv_obj_t *obj_text = lv_label_create(btn);
		if (sty_text)
			lv_obj_add_style(obj_text, sty_text, LV_PART_MAIN);
		lv_label_set_long_mode(obj_text, LV_LABEL_LONG_SCROLL_CIRCULAR);
		lv_label_set_text(obj_text, text);
		//lv_obj_set_flex_grow(obj_text, 1);
	}

	return btn;
}

lv_obj_t * watch_list_add_btn3(lv_obj_t *obj, const lv_img_dsc_t * icon_src, const char * text, lv_style_t *sty_text)
{
	watch_list_t *list = (watch_list_t *)obj;
	lv_obj_t *btn = lv_btn_create(obj);
...
static void watch_list_draw(lv_event_t * e)
{
	...
	if (code == LV_EVENT_DRAW_MAIN) {
		...
		if (disp->driver->gpu_fill_cb) {
			...
			res = disp->driver->gpu_fill_cb(disp->driver, disp_buf,
					lv_area_get_width(disp_area), draw_dsc.bg_color, draw_dsc.bg_opa,
