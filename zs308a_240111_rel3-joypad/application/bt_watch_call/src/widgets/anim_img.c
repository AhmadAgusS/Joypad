...
void anim_img_set_src_picarray(lv_obj_t *obj, const lv_img_dsc_t * src, const lv_point_t * pos, uint16_t cnt)
{
	anim_img_t *animimg = (anim_img_t *)obj;
	...
	anim_img_clean(obj);
	/* initialize loader */
	lvgl_img_loader_set_picarray(&animimg->loader, src, pos, cnt);
...
static bool load_image(lv_obj_t *obj, uint16_t index)
{
	...
	if (lvgl_img_loader_load(&animimg->loader, index, &next_src, &pos) == 0) {
		...
		simple_img_set_src(obj, &animimg->src_dsc);
