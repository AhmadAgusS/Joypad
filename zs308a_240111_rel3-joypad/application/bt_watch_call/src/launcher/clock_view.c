...
static uint32_t clock_scene_id = 0;
...
static int _clock_view_load_resource(clock_view_data_t *data, clock_view_tmp_res_t *tmp_res, bool first_layout)
{
	...
	/* scene */
	ret = lvgl_res_load_scene(clock_scene_id, &data->scene,
			DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
	if (ret < 0) {
		SYS_LOG_ERR("scene 0x%x not found", clock_scene_id);
		return -ENOENT;
	}
...
#else
			ret = lvgl_bitmap_font_open(&data->font, DEF_FONT28_FILE);
			if (ret < 0) {
				return -ENOENT;
			}
#endif
...
{
	lvgl_res_preload_cancel_scene(clock_scene_id);
	lvgl_res_unload_scene_compact(clock_scene_id);
...
static int _clock_view_preload(view_data_t *view_data, bool update)
{
	...
	clock_scene_id = presenter->get_clock_id ? presenter->get_clock_id() : SCENE_CLOCK_VIEW;

	//lvgl_res_set_pictures_regular(clock_scene_id, 0, 0, regular_id, 2, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);

	if (lvgl_res_preload_scene_compact_default(clock_scene_id, CLOCK_VIEW, update, 0)) {
		clock_scene_id = 0;
		return -ENOMEM;
	}
...
static int _clock_view_layout_update(view_data_t *view_data, bool first_layout)
{
	...
	if (first_layout) {
		...
		if (clock_scene_id == 0) { /* preload may bypass */
			clock_scene_id = presenter->get_clock_id ? presenter->get_clock_id() : SCENE_CLOCK_VIEW;
		}
	}
	...
	if (data->grp_weather.width > 0) {
		data->obj_info[INFO_WEATHER] = simple_img_create(data->obj_clock);
	...
	if (data->prg_anim.width > 0) {
		...
		if (data->obj_info[INFO_ANIM]) {
			...
			anim_img_set_src_picregion(data->obj_info[INFO_ANIM], clock_scene_id, &data->prg_anim);
...
static int _clock_view_paint(view_data_t *view_data, bool forced)
{
	...
	if (data->obj_info[INFO_DATE]) {
		if (forced || (data->tm_mday != time.tm_mday && data->tm_mon != time.tm_mon &&
	...
	/* update weather */
	if (data->obj_info[INFO_WEATHER]) {
		value = presenter->get_weather();
		if (forced || data->weather != value) {
			lv_img_dsc_t img_weather;

			if (0 == lvgl_res_load_pictures_from_group(&data->grp_weather,
					&pic_weather_ids[value], &img_weather, NULL, 1)) {
				data->weather = value;
				lvgl_res_unload_pictures(&data->img_weather, 1);
				memcpy(&data->img_weather, &img_weather, sizeof(img_weather));
				simple_img_set_src(data->obj_info[INFO_WEATHER], &data->img_weather);
			}
		}
	}
...
static int _clock_view_focus_changed(view_data_t *view_data, bool focused)
{
	...
	if (focused) {
		if (data) {
			if (!lvgl_res_scene_is_loaded(clock_scene_id)) {
