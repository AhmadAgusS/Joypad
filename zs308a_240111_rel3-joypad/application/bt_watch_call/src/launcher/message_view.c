...
#include "widgets/msg_list.h"
#include "widgets/msg_box.h"

//LOG_MODULE_DECLARE(message);
...
typedef struct message_view_tmp_res {
...
typedef struct message_view_data {
	lv_obj_t *list;
...
static int32_t message_preload_inited = 0;
...
static int _message_view_load_resource(message_view_data_t *data, message_view_tmp_res_t *res, bool first_layout)
{
    ...
    if(first_layout)
    {
	    ret = lvgl_res_load_scene(SCENE_MSG_VIEW, &data->res_scene, DEF_STY_FILE, DEF_RES_FILE, DEF_STR_FILE);
	    if (ret < 0) return ret;
	}

	ret = lvgl_res_load_group_from_scene(&data->res_scene, RES_GRP_LIST, &data->res_list);
	if (ret < 0) return ret;

	ret = lvgl_res_load_picregion_from_group(&data->res_list, RES_ICON, &picregion);
	if (ret < 0) {
		lvgl_res_unload_group(&data->res_list);
		return ret;
	}
	...
	if(first_layout)
	{
		ret = lvgl_res_load_group_from_group(&data->res_list, RES_PREVIEW, &res->res_list_preview);
		if (ret < 0) {
			lvgl_res_unload_group(&data->res_list);
			return ret;
		}
	...
	if(first_layout)
	{
		...
		ret = lvgl_res_load_group_from_scene(&data->res_scene, RES_GRP_CONTENT, &data->res_content);
		if (ret < 0) return ret;

		ret = lvgl_res_load_strings_from_group(&data->res_content, res_content_str_ids, res->res_content_str, ARRAY_SIZE(res_content_str_ids));
		if (ret < 0) {
			lvgl_res_unload_group(&data->res_content);
			return ret;
		}

		lvgl_res_unload_strings(res->res_content_str, ARRAY_SIZE(res->res_content_str));

		ret = lvgl_res_load_group_from_group(&data->res_content, RES_ICON, &res_subgrp);
		if (ret < 0) {
			lvgl_res_unload_group(&data->res_content);
			return ret;
		}
...
#if DEF_UI_WIDTH < 454
		ret = lvgl_bitmap_font_open(&data->font, DEF_FONT24_FILE);
		if (ret < 0) return ret;

		ret = lvgl_bitmap_font_open(&data->font_small, DEF_FONT24_FILE);
		if (ret < 0) return ret;
#else
		ret = lvgl_bitmap_font_open(&data->font, DEF_FONT32_FILE);
		if (ret < 0) return ret;

		ret = lvgl_bitmap_font_open(&data->font_small, DEF_FONT28_FILE);
		if (ret < 0) return ret;
#endif

#ifdef CONFIG_BITMAP_FONT_SUPPORT_EMOJI 		
		lvgl_bitmap_font_set_emoji_font(&data->font, DEF_FONT28_EMOJI);
		lvgl_bitmap_font_set_emoji_font(&data->font_small, DEF_FONT28_EMOJI);
		lvgl_bitmap_font_set_default_code(&data->font, 0x53E3, 0x1ffff);
		lvgl_bitmap_font_set_default_code(&data->font_small, 0x53E3, 0x1ffff);
#endif
	}
...
static void _message_view_unload_resource(message_view_data_t *data)
{
	...
	lvgl_bitmap_font_close(&data->font_small);
...
static int _show_message_list(view_data_t *view_data)
{
	...
	data->list = msg_list_create(scr);
	lv_obj_set_pos(data->list, data->res_list.x, data->res_list.y);
	lv_obj_set_size(data->list, data->res_list.width, data->res_list.height);

	msg_list_set_layout(data->list, &data->sty_list_app_icon, NULL,
			&data->sty_list_name, &data->sty_list_text, NULL, false);

	for (int i = 0; i < ARRAY_SIZE(messages); i++) {
		lv_obj_t *btn = msg_list_add(data->list, &data->img_icons[messages[i].type],
				NULL, messages[i].name, messages[i].text, NULL);
...
static int _show_message_content(view_data_t *view_data, const message_desc_t *msg)
{
	...
	data->box = msg_box_create(scr);
	lv_obj_set_pos(data->box, data->res_content.x, data->res_content.y);
	lv_obj_set_size(data->box, data->res_content.width, data->res_content.height);

	msg_box_set_layout(data->box, &data->sty_box_app_icon, NULL, &data->sty_box_name, &data->sty_box_text, false);
	msg_box_set_max_height(data->box, lv_obj_get_height(scr) * MSG_CONTENT_PAGE_NUM);
	msg_box_set_text(data->box, &data->img_icons[msg->type], NULL, msg->name, msg->text);

	/* refresh the visible content ASAP */
	lv_refr_now(view_data->display);

	lv_obj_add_event_cb(data->box, _message_content_event_handler, LV_EVENT_GESTURE, data);

	/* set system gesture */
	ui_manager_gesture_set_dir(0);

	/* Refresh ASAP */
	ui_view_refresh(MSG_VIEW);
	return 0;
}
...
static int _message_view_layout_update(view_data_t *view_data, bool first_layout)
{
	...
	if(first_layout)
	{
		...
		lv_style_set_width(&data->sty_list_btn, tmp_res.res_list_preview.width);
		lv_style_set_height(&data->sty_list_btn, tmp_res.res_list_preview.height);

		lv_style_init(&data->sty_list_name);
		lv_style_set_x(&data->sty_list_name, tmp_res.res_list_str[0].x);
		lv_style_set_y(&data->sty_list_name, tmp_res.res_list_str[0].y);
		...
		lv_style_set_x(&data->sty_list_text, tmp_res.res_list_str[1].x);
		lv_style_set_y(&data->sty_list_text, tmp_res.res_list_str[1].y);
...
static int _message_view_updated(view_data_t* view_data)
{
	...
	ret = _message_view_layout_update(view_data, false);
