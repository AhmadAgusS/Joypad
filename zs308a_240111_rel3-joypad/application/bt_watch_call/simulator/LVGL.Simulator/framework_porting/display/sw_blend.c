...
void sw_blend_color_over_rgb565(void *dst, uint32_t src_color,
		uint16_t dst_stride, uint16_t w, uint16_t h)
{
	uint16_t *dst16 = dst;
		...
		uint16_t dst_stride, uint16_t src_stride, uint16_t w, uint16_t h)
{
	...
	uint16_t *dst16 = dst;
	uint16_t src_y_step = src_stride;
	uint16_t src_x_step = 1;
	...
	for (int j = h; j > 0; j--) {
		...
		for (int i = w; i > 0; i--) {
			uint32_t color32 = src_color | ((uint32_t)*tmp_src8 << 24);
		...
		uint16_t dst_stride, uint16_t src_stride, uint16_t w, uint16_t h)
{
	...
	for (int j = h; j > 0; j--) {
		...
		for (int i = w; i > 0; i--) {
			*tmp_dst = blend_argb8565_over_rgb565(*tmp_dst, tmp_src8);
			tmp_dst++;
			tmp_src8 += src_x_step;
		}
		...
		uint16_t dst_stride, uint16_t src_stride, uint16_t w, uint16_t h)
{
	...
	for (int j = h; j > 0; j--) {
		...
		for (int i = w; i > 0; i--) {
			*tmp_dst = blend_argb8565_over_argb8888(*tmp_dst, tmp_src8);
			tmp_dst++;
			tmp_src8 += src_x_step;
		}
		...
		uint16_t dst_stride, uint16_t src_stride, uint16_t w, uint16_t h)
{
	...
	for (int j = h; j > 0; j--) {
		...
		for (int i = w; i > 0; i--) {
			*tmp_dst = blend_argb6666_over_argb8888(*tmp_dst, tmp_src8);
			tmp_dst++;
			tmp_src8 += src_x_step;
		}
